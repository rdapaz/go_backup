...
func main() {
	...
	flag.StringVar(&profile, "profile", ProfileDocuments, "Backup profile: documents | jetbrains | databases")
	...
	if srcDir == "" || dstDir == "" {
		fmt.Fprintf(os.Stderr, "Usage: %s -src <sourceDir> -dst <destDir> -profile <documents|jetbrains|databases> [-password <pwd>] [-hint <hint>] [-workers N] [--keep-stage]\n", os.Args[0])
		os.Exit(1)
	}

	switch profile {
	case ProfileDocuments, ProfileJetBrains, ProfileDatabases:
	default:
		fmt.Fprintf(os.Stderr, "Invalid profile: %s (expected documents|jetbrains|databases)\n", profile)
		os.Exit(1)
	}
